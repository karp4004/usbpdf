...
""".trimIndent()
        ...
        for ((objId, content) in allObjects) {
            ...
            if (actionRef != null) {
                ...
                val actionObject = allObjects[actionRef.toIntOrNull()]
    ...
    private fun extractTextFallbackLinks(
        ...
        objects.values.forEach { obj ->
            Regex("""/Font\s*<<(.*?)>>""", RegexOption.DOT_MATCHES_ALL)
                ...
                .forEach { fontDict ->
                    fontRefRegex.findAll(fontDict.groupValues[1]).forEach { ref ->
                        ref.groupValues[2].toIntOrNull()?.let {
                            fontRefs[ref.groupValues[1]] = it
                        }
                    }
                }
            ...
            Regex("""/(R\d+)\s+(\d+)\s+\d+\s+R""")
                .findAll(obj)
                .forEach { ref -> ref.groupValues[2].toIntOrNull()?.let { fontRefs[ref.groupValues[1]] = it } }
        }
        ...
        pageMap.forEach { (pageObjId, pageIndex) ->
            ...
            if (contents != null) {
                ...
                Regex("""(\d+)(?:\s+\d+\s+R)?""").findAll(refsText).forEach {
                    ...
                    id?.let { contentToPage[id] = pageIndex }
                    ...
                    objects[id]?.let { contentsObject ->
                        Regex("""(\d+)\s+\d+\s+R""").findAll(contentsObject).forEach { child ->
                            child.groupValues[1].toIntOrNull()?.let {
                                contentToPage[it] = pageIndex
                            }
                        }
                    }
                }
    ...
    private fun collectRequiredStreamIds(objects: Map<Int, String>): Set<Int> {
        ...
        objects.values.forEach { obj ->
            Regex("""/ToUnicode\s+(\d+)\s+\d+\s+R""")
                .findAll(obj)
                .forEach { it.groupValues[1].toIntOrNull()?.let { result.add(it) } }
        }
        ...
        objects.values.forEach { obj ->
            Regex(
                ...
                if (refs.isNotBlank()) {
                    refRegex.findAll(refs).forEach { queue.add(it.groupValues[1].toIntOrNull()) }
                } else {
        ...
        while (queue.isNotEmpty()) {
            ...
            if (!obj.contains("stream")) {
                refRegex.findAll(obj).forEach { queue.add(it.groupValues[1].toIntOrNull()) }
            }
        }
        return result
    }
    ...
    private fun extractCMap(
        ...
        Regex(
            ...
            val sourceStart = match.groupValues[1].toIntOrNull(16) ?: return@forEach
            val sourceEnd = match.groupValues[2].toIntOrNull(16) ?: return@forEach
